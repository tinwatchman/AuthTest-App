...
package feathers.controls
{
	...
	public class Header extends FeathersControl
	{
		...
		protected function layoutTitle():void
		{
			...
			if(this._titleAlign == TITLE_ALIGN_PREFER_LEFT)
			{
				// just my own personal edge case
				const totalTitleGap:Number = (rightOffset > 0) ? (calculatedTitleGap*2) : calculatedTitleGap;
				this._titleRenderer.maxWidth = this.actualWidth - this._paddingLeft - leftOffset - rightOffset - totalTitleGap;
				this._titleRenderer.validate();
				this._titleRenderer.x = this._paddingLeft + leftOffset + calculatedTitleGap;
			}
